...
function Search() {
  ...
  const { getProductByTitle } = useContext(ProductContext);

  const doSearch = useDebounce(async (term) => {
    getProductByTitle(term);
  }, 500);

  function handleChange(e) {
    const value = e.target.value; // Correctly fetch the input value
    setText(value); // Update the text state with the input value
    doSearch(value); // Trigger the debounced search function
  }
